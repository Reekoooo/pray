...
void main() {
  ...
  setUp(() {
    ...
    locationDataSource = LocationDataSourceImpl(
        geoLocator: geoLocatorMock, permissionHandler: permissionHandlerMock);
  });
  ...
  group('geolocation status null', () {
    ...
    setUp(() {
      ...
      when(geoLocatorMock.checkGeolocationPermissionStatus())
          .thenAnswer((_) => Future.value(geoLocationStatus));
    });

    test('should request location permission when geolocation status is null',
        () async {
      final permissionStatus = {
        PermissionGroup.location: PermissionStatus.granted
      };
      when(permissionHandlerMock.requestPermissions(any))
      ...
      verify(
          permissionHandlerMock.requestPermissions([PermissionGroup.location]));
    });

    test("should return Device Location when permission status is granted",
        () async {
      final permissionStatus = {
        PermissionGroup.location: PermissionStatus.granted
      };
    ...
    test('should throw exception when permission status is denied', () async {
      final permissionStatus = {
        PermissionGroup.location: PermissionStatus.denied
      };
  ...
  group('geolocation status is not null', () {
    ...
    test(
        'should not request location permission when geolocation status is not null',
        () async {
      when(geoLocatorMock.getCurrentPosition(
              desiredAccuracy: anyNamed('desiredAccuracy')))
          .thenAnswer((_) => Future.value(expected.toPosition()));
